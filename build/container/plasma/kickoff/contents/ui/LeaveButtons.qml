...
RowLayout {
    ...
    component FilteredModel : KItemModels.KSortFilterProxyModel {
        ...
        function sourceFavoriteId(sourceRow, sourceParent) {
            const FavoriteIdRole = sourceModel.KItemModels.KRoleNames.role("favoriteId");
            return sourceModel.data(sourceModel.index(sourceRow, 0, sourceParent), FavoriteIdRole);
        }

        function systemFavoritesContainsRow(sourceRow, sourceParent) {
            return String(Plasmoid.configuration.systemFavorites).includes(sourceFavoriteId(sourceRow, sourceParent));
        }

        function isDisconnectAction(sourceRow, sourceParent) {
            return sourceFavoriteId(sourceRow, sourceParent) === "logout";
        }
    ...
    FilteredModel {
        ...
        filterRowCallback: (sourceRow, sourceParent) => isDisconnectAction(sourceRow, sourceParent)
    }
